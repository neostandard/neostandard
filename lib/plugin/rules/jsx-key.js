...
// (`arr.map(renderRow)`), imperatively-built arrays, `try`/`switch`/loops, and —
// for array literals only — `LogicalExpression`/`ConditionalExpression` elements
// (`[x || <li/>]`, `[x ? <li/> : <span/>]`) are not unwrapped the way
// iterator-callback returns are.
  ...
  create (context) {
    ...
    function checkReturnedExpression (expression) {
      if (expression.type === 'ConditionalExpression') {
        checkReturnedExpression(expression.consequent)
        checkReturnedExpression(expression.alternate)
      } else if (expression.type === 'LogicalExpression') {
        checkReturnedExpression(expression.right)
      } else {
        reportIfKeyless(expression, 'iter')
      }
    }
    ...
    function walkReturnedJSX (body) {
      if (body.type === 'BlockStatement') {
        ...
        for (const expression of returns) checkReturnedExpression(expression)
      } else {
        checkReturnedExpression(body)
      }
    }
    ...
    return {
      ArrayExpression (node) {
        ...
        for (const element of node.elements) {
          if (element != null) reportIfKeyless(element, 'array')
        }
      },
      CallExpression (node) {
        if (isWithinChildrenToArray(node)) return
