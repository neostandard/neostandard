...
**neostandard** is a modern successor to `standard` — an ESLint **shareable config** distributed as a flat-config (ESLint 9/10) array. It is plain JavaScript (CommonJS) typed with JSDoc; `tsc` only type-checks and emits `.d.ts` declarations — there is **no compile step**, the `.js` you edit is the `.js` that ships.
...
- **`index.js`** — public surface. Re-exports the `neostandard` function, `resolveIgnoresFromGitignore()`, and a `plugins` object. `plugins` uses **lazy getters** (`@stylistic`, `n`, `promise`, `neostandard`, `typescript-eslint`) so consumers can layer extra config using the *exact same plugin instances* neostandard uses, without eager `require()` cost. (`react` is omitted while `eslint-plugin-react` is incompatible with ESLint 10 — issue #350; the one reimplemented rule is exposed via the `neostandard` getter — see `lib/plugin/` below.)
...
- Peer dependency is currently `eslint: ^9.22.0 || ^10.0.0` (the `9.22` floor is where `eslint/config`'s `defineConfig` landed, which the README's extending examples use). Releases are automated via **release-please** — don't hand-edit `CHANGELOG.md` or bump `version`.
