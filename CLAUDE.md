...
- **`lib/main.js`** — the composer. Resolves `env`/`globals` (via the `globals` package), then assembles the array in this order: lone-key `{ ignores }` first (must be alone to count as a *global* ignore per ESLint), then `neostandard/additional-files`, then `neostandard/globals`, then the JS layers, then a single TypeScript block last. Throws if `filesTs` is passed without `ts`.
